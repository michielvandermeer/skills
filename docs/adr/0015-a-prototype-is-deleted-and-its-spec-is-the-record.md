...
- The worktree levers are host-shaped exactly as [ADR-0014](0014-implement-worktree-host-agnostic.md) sets them, and the fallback path is `.agents/worktrees/prototype/<slug>`. A fresh worktree carries no installed dependencies, so a UI prototype installs the project's before rule 2's one command works.
