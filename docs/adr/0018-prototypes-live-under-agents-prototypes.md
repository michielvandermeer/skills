...
- `/to-spec` names `.agents/prototypes/<slug>/`. Decision-rich snippets (a reducer, a machine) may still be inlined; the folder is the playable record. A Refinement used to name the same path; that line is superseded by [ADR-0025](0025-refine-prototype-lives-in-the-refinement-folder.md).
...
- ADR-0016 is superseded for the "nothing survives" rule. The worktree, the host-shaped levers, the no-fold-into-real-code rule, and the Spec-as-handover rule stand.
