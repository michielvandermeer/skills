...
This restores auto-start, which [ADR-0032](0032-retro-is-a-typed-command.md) and [ADR-0022](0022-sessions-end-when-the-skill-you-typed-is-done.md) had banned, and goes further than [ADR-0019](0019-implement-ends-with-a-non-blocking-retrospective.md), which only presented.

## Consequences

- ADR-0032's auto-run ban is superseded. The typed command stays.
- ADR-0022's wrap-up ban is superseded for these skills. A session whose typed skill is on the list ends when the retrospective summary is in the chat.
