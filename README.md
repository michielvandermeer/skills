...
- A **spec-bound dispatch** works to a document settled before it started, so it runs at `effort: medium` on your session's model — `skills:implementer`, `skills:checker`, `skills:oneshot`, `skills:explorer`, and `skills:researcher`.
