...
Each layout below predates a decision that changed where documents live. All of them are **mechanical moves**: the destination follows from the source path, so relocate the tree rather than classifying file by file. Move, then fix references, the same as a classified document — and a collision at the destination is handled the same way: leave it, and list it in the report.
...
## A foldered Refinement

`.agents/refinements/<slug>/` follows the Refinement-becomes-an-Idea rule in [SKILL.md](SKILL.md#refinement-becomes-an-idea): `complete.md` (or `session.md` when there's no `complete.md`) becomes `.agents/ideas/<slug>.md`, its `prototype/` moves to `.agents/prototypes/<slug>/`, and the emptied folder is removed.
