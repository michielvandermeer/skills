...
A leading word earns its density in the source, which only the agent reads. What a skill puts in front of a **person** runs at the bar the `/plain-language` skill sets — dense source, plain output.
