...
3. **Resolve each hunk.** Preserve both intents where possible. Where they cannot both stand, pick the one matching the merge's stated goal and note the trade-off. Use only behaviour already on one side or the other. Always resolve; never `--abort`.
