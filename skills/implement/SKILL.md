...
7. Call the `ExitWorktree` tool with `action: "keep"` to return the session to the original directory, preserving the branch and its commits. (Merging must happen from the original directory, since the target branch — e.g. master — stays checked out there and can't be merged into from within the worktree.)
8. From the original directory, merge the session's branch into master, then remove the worktree and delete the branch (`git worktree remove <path>` and `git branch -d <branch>`).
9. Provide a summary of all the work in this session to the user.